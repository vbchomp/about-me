...
// Rules
alert(`Good Day, ${username}! Want to learn a little about me? Please answer Yes or No to the first five questions below. The last 2 need numbers. Thanks for playing!`);
...
// Question 2
function q2() {
  let q2 = prompt('Do I love animals?').toLowerCase();
  // if loop
  if (q2 === 'no' || q2 === 'n') {
...
function q7() {
  while (placeAttempts && !correctGuess){
    let question7 = prompt(`What are a few of the best places to visit in WA state? I only added 5 just to be nice. You only have ${placeAttempts} guesses.`);
    // decrements guesses
    placeAttempts--;
    for (let i = 0; i < waPlaces.length; i++){
      //sets correctanswer to true if one of the correct answers is given
      if (question7 === waPlaces[i]){
        correctGuess = true;
      }
    }
  }
